(ns toysmfg.operation-test
  "Smoke tests for the compiled ToysGamesOperationActor graph itself
  (build + one happy path per op). The governor's full rule contract
  (HARD holds, escalation, phase gating) is exercised in
  `toysmfg.governor-contract-test`; the Store contract in
  `toysmfg.store-contract-test`."
...
(deftest test-actor-builds
  (testing "ToysGamesOperationActor can be built with a store"
    ...
    (let [s (-> (store/mem-store) (store/sample-data!))
          ...
          result (exec-op actor "t3"
                          {:op :flag-safety-concern :effect :propose :subject "concern-1"
                           :value {:equipment-id "molding-001" :severity :moderate :description "small-parts choking-hazard flagged"}}
                          coordinator)]
      (is (= :interrupted (:status result))))))
