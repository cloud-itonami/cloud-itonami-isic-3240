(ns toysmfg.store-contract-test
  "The Store contract as executable tests. Single MemStore backend --
  see `toysmfg.store` ns docstring for why a second (Datomic-backed)
...
(deftest batch-upsert-merges-preserving-untouched-fields
  (let [s (seeded)]
    (store/commit-record! s {:effect :batch/upsert :path ["batch-001"]
                             :value {:product-type :wooden-toy}})
    (is (= :wooden-toy (:product-type (store/batch s "batch-001"))))
