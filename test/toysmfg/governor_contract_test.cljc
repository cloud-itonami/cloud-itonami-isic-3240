(ns toysmfg.governor-contract-test
  ...
  operations... does NOT self-issue a toy-safety compliance
  certification') implemented faithfully. The single invariant under
  test:

    ToysGamesAdvisor never schedules maintenance, flags a safety
    concern, or coordinates a shipment the Toys & Games Plant
  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-production-batch :effect :propose :subject "batch-001"
                   :patch {:product-type :plastic-toy}} coordinator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= :plastic-toy (:product-type (store/batch db "batch-001"))) "SSoT actually updated")
...
(deftest safety-certification-authority-is-held-and-permanently-blocked
  (testing "a proposal that sets :issue-safety-certification? true -> HOLD, PERMANENT, never reaches request-approval -- this actor is never the toy-safety-certification authority"
    (let [[db actor] (fresh)
...
(deftest invalid-product-type-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10" {:op :log-production-batch :effect :propose :subject "batch-001"
                                  :patch {:product-type :unobtainium-figurine}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-product-type} (-> (store/ledger db) last :basis)))
    (is (not= :unobtainium-figurine (:product-type (store/batch db "batch-001"))) "fabricated product-type never lands in the SSoT")))

(deftest invalid-safety-test-pass-percent-is-held
  (let [[db actor] (fresh)
        res (exec-op actor "t10b" {:op :log-production-batch :effect :propose :subject "batch-001"
                                   :patch {:safety-test-pass-percent 250}} coordinator)]
    (is (= :hold (get-in res [:state :disposition])))
    (is (some #{:invalid-safety-test-pass-percent} (-> (store/ledger db) last :basis)))
    (is (not= 250 (:safety-test-pass-percent (store/batch db "batch-001"))) "fabricated safety-test-pass-percent never lands in the SSoT")))
