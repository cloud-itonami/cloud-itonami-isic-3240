(ns toysmfg.registry-test
  (:require [clojure.test :refer [deftest is]]
            [toysmfg.registry :as r]))
...
(deftest small-shipment-within-quantity-does-not-exceed
  (is (false? (r/shipment-quantity-exceeded?
               {:quantity-units 800.0 :shipped-units 200.0} 50.0))))
...
(deftest known-product-types-are-valid
  (doseq [m [:plastic-toy :wooden-toy :board-game :puzzle]]
    (is (r/product-type-valid? m))))

(deftest fabricated-product-type-is-invalid
  (is (not (r/product-type-valid? :unobtainium-figurine)))
  (is (not (r/product-type-valid? nil))))

;; ----------------------------- safety-test-pass-percent-valid? -----------------------------

(deftest typical-safety-test-pass-percent-is-valid
  (is (r/safety-test-pass-percent-valid? 1))
  (is (r/safety-test-pass-percent-valid? 96))
  (is (r/safety-test-pass-percent-valid? 100)))

(deftest below-floor-safety-test-pass-percent-is-invalid
  (is (not (r/safety-test-pass-percent-valid? 0)))
  (is (not (r/safety-test-pass-percent-valid? -1))))

(deftest excessive-safety-test-pass-percent-is-invalid
  (is (not (r/safety-test-pass-percent-valid? 250)))
  (is (not (r/safety-test-pass-percent-valid? 101))))

(deftest non-integer-or-missing-safety-test-pass-percent-is-invalid
  (is (not (r/safety-test-pass-percent-valid? nil)))
  (is (not (r/safety-test-pass-percent-valid? 92.5)))
  (is (not (r/safety-test-pass-percent-valid? "92"))))

;; ----------------------------- weight-grams-valid? -----------------------------

(deftest typical-weight-grams-is-valid
  (is (r/weight-grams-valid? 0.5))
  (is (r/weight-grams-valid? 96000.0))
