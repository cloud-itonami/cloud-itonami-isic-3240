(ns toysmfg.registry
  "Pure-function domain logic for the games-and-toys plant-operations
  coordination actor -- equipment/batch verification, shipment-
  quantity recompute, product-type validation, safety-test-pass-
  percent plausibility validation, batch-weight plausibility
  ...
  pre-existing `kotoba-lang/toysmfg`-style capability library to wrap
  (verified: no such repo exists, and no `toy`/`toys`/`game`/`games`-
  ...
  as pure functions, re-verified INDEPENDENTLY by `toysmfg.governor`
  -- the same 'ground truth, not self-report' discipline every
  sibling actor's own registry establishes (most directly
  `sportsgoodsmfg.registry` from `cloud-itonami-isic-3230`, this
  vertical's closest domain analog -- both are back-office
  coordination actors for a molding/assembly plant with a real
  consumer-protection dimension): never trust a proposal's own
  self-reported quantity/status when the inputs needed to recompute
  it independently are already on record.

  This namespace is pure data + pure functions -- no I/O, no network
  call to any real plant-operations system. It builds the DRAFT
  record a plant coordinator would keep (a scheduled equipment
  maintenance window, a coordinated shipment), not the act of
  actuating molding/assembly/finishing-line equipment or dispatching
  a real freight carrier, and never the act of self-issuing a
  toy-safety compliance certification (this actor NEVER does any of
  those -- see README `What this actor does NOT do`).

  SCOPE: ISIC 3240 covers manufacture of games and toys -- plastic and
  wooden toys, board games, and puzzles produced on injection-
  molding/assembly/safety-test lines. This actor coordinates the
  ...
  equipment directly, and it never stands in for the toy-safety-
  certification authority (e.g. an accredited testing/certification
  body applying ASTM F963 / EN 71) that certifies a product's
  compliance with an applicable toy-safety standard."
  )
...
(def valid-product-types
  ...
  declare -- the games and toys routinely molded/assembled/safety-
  tested on a plant's production lines. Anything else is a
  ...
  never DECIDES a batch's product type (that is the plant's own
  production-intake function); it only validates that a batch record
  declares one of the real, known values."
  #{:plastic-toy :wooden-toy :board-game :puzzle})

(def safety-test-pass-percent-min
  "Physical floor for a batch's own toy-safety compliance-test
  reading, expressed as a percentage of the applicable toy-safety
  standard's own pass threshold (e.g. the choking-hazard/small-parts/
  lead-paint/phthalate compliance percentage a testing lab would
  report for a plastic-toy, wooden-toy, board-game, or puzzle batch,
  per ASTM F963 / EN 71 or an equivalent national/regional standard).
  A reading of 0 or below is not a real compliance-test result."
  1)

(def safety-test-pass-percent-max
  "Physical ceiling for a batch's own toy-safety compliance-test
...
(def weight-grams-max
  ...
  batch across this vertical's product types (e.g. ~500 large
  ride-on-toy playsets is already a large plastic-toy batch); a
  reading beyond this is implausible sensor/scale data, not a real
  batch."
  1000000.0)

(def defect-rate-min-percent
  "Physical floor for a batch's own molding/assembly/safety-test
...
(defn batch-verified?
  ...
  (i.e. its product-type/safety-test-pass-percent/weight/defect-rate
...
(defn product-type-valid?
  "Is `product-type` one of the closed, known games-and-toys values?
...
(defn safety-test-pass-percent-valid?
  "Is `rating` a physically/certifiably plausible toy-safety
  compliance-test reading, expressed as a percentage of the
  applicable standard's own pass threshold (ASTM F963 / EN 71 or
  equivalent)? Rejects nil, non-integers, values below
  `safety-test-pass-percent-min`, and values beyond
  `safety-test-pass-percent-max` -- a fabricated or sensor-error
  reading, never let through as a real compliance-test fact."
  [rating]
  (and (integer? rating)
       (>= rating safety-test-pass-percent-min)
       (<= rating safety-test-pass-percent-max)))
...
(defn defect-rate-valid?
  "Is `percent` a physically plausible batch molding/assembly/
  safety-test defect-rate reading? Rejects nil, non-numbers, negative
...
(defn- unsigned-certificate
  ...
  actor's. And NEVER a toy-safety compliance certification -- this
  actor is never the toy-safety-certification authority (see README
...
(defn register-maintenance
  ...
  the molding/assembly-line equipment or execute any maintenance; it
  builds the RECORD a plant coordinator would keep. `toysmfg.governor`
  ...
  actuate molding/assembly-line equipment (see README `Actuation`),
...
(defn register-shipment
  ...
  outbound games-and-toys shipment against a verified, registered
  production batch. Pure function -- does not dispatch any real
  freight carrier; it builds the RECORD a plant coordinator would
  keep. `toysmfg.governor` independently re-verifies the shipment's
