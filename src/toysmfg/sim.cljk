(ns toysmfg.sim
  ...
  a proposal that tries to self-issue a toy-safety compliance
  ...
  implausible safety-test-pass-percent reading, a production-batch
  ...
  (let [db (-> (store/mem-store) (store/sample-data!))
    ...
    (println (exec-op actor "t5"
                       {:op :log-production-batch :effect :direct-write :subject "batch-001"
                        :patch {:product-type :plastic-toy}}
                       coordinator))
    ...
    (println (exec-op actor "t12"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:product-type :unobtainium-figurine}}
                       coordinator))

    (println "== log-production-batch batch-001 with an implausible safety-test-pass-percent -> HARD hold ==")
    (println (exec-op actor "t13"
                       {:op :log-production-batch :effect :propose :subject "batch-001"
                        :patch {:safety-test-pass-percent 250}}
                       coordinator))
    ...
    (println "== log-production-batch batch-001 attempting to self-issue a toy-safety compliance certification -> HARD hold, PERMANENT ==")
