(ns toysmfg.operation
  "ToysGamesOperationActor -- one plant-operations coordination
  request = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (ToysGamesAdvisor) is sealed into a single
  node (:advise); its proposal is ALWAYS routed through the Toys &
  Games Plant Operations Governor (:govern) and the rollout phase gate
...
(defn build
  "Compiles a ToysGamesOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `toysmfg.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; ToysGamesAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Toys & Games Plant Operations Governor -- independent censor.
      (g/add-node :govern
