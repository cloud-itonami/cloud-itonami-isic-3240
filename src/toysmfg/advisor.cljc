(ns toysmfg.advisor
  "ToysGamesAdvisor -- the *contained intelligence node* for the
  games-and-toys plant-operations coordination actor.

  It normalizes production-batch patches (product-type/safety-test-
  pass-percent/weight-grams/defect-rate), drafts a molding/assembly-
  ...
  games-and-toys shipment coordination proposal against a production
  batch. CRITICAL: it is a smart-but-untrusted advisor. It returns a
  *proposal* (with a rationale + the fields it cited), never a
  committed record and NEVER a real molding/assembly-line-equipment
  actuation, freight dispatch, or toy-safety compliance certification
  -- see README `What this actor does NOT do`. Every output is
  censored downstream by `toysmfg.governor` before anything touches
  the SSoT.
  ...
  contract every caller of this actor agrees to) -- `toysmfg.governor`
...
(defn- log-production-batch
  ...
  safety-test-pass-percent, weight-grams, or defect-rate, nor its
...
(defn- schedule-maintenance
  ...
  `toysmfg.governor` NEVER trusts this report -- it independently
...
(defn- flag-safety-concern
  "Draft a choking-hazard/materials-safety (e.g. lead paint, phthalate)
  / small-parts concern. ALWAYS `:stake :coordination/safety-concern`
  ...
  administrative technicality). See `toysmfg.phase`: no phase ever
  adds this op to a phase's `:auto` set; `toysmfg.governor` also
...
(defn- coordinate-shipment
  "Draft an outbound games-and-toys shipment coordination proposal
  against a production batch. The advisor passes through the caller's
  own claimed unit quantity -- it does NOT invent one, and
  `toysmfg.governor` NEVER trusts it: it independently recomputes
...
(def ^:private system-prompt
  (str "You are the advisor for a games-and-toys plant-operations "
       ...
       "executes). Never propose self-issuing a toy-safety compliance "
...
(defn- parse-proposal
  ...
  yields a safe low-confidence noop so `toysmfg.governor`
