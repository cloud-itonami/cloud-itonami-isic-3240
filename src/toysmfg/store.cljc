(ns toysmfg.store
  "SSoT for the games-and-toys plant-operations coordination actor,
  ...
  3230`'s own `sportsgoodsmfg.store`), this build ships a single
  ...
  games-and-toys-manufacturing capability library, no
  ...
  Four kinds of entity live here:
    - `batches`          -- the central entity. A production batch's
                             product-type/safety-test-pass-percent/
    ...
    - `maintenance`       -- a scheduled molding/assembly-equipment
                             ...
                             of equipment (`toysmfg.registry`'s
    ...
    - `shipments`         -- a proposed outbound games-and-toys
                             shipment DRAFT (`toysmfg.registry`'s
                             `register-shipment`).
...
(defprotocol Store
  ...
  (ledger [s])
  (maintenance-history [s] "the append-only maintenance-schedule history (toysmfg.registry drafts)")
  (shipment-history [s] "the append-only shipment-coordination history (toysmfg.registry drafts)")
...
(defn- sample-batches []
  {"batch-001" {:id "batch-001" :product-type :plastic-toy
                :lot-number "TOY-2026-001" :safety-test-pass-percent 96
                :quantity-units 800.0 :weight-grams 96000.0
                :defect-rate-percent 0.6
                :verified? true :registered? true
                :shipped-units 200.0
                :last-assessed "2026-06-01"}
   "batch-002" {:id "batch-002" :product-type :board-game
                :lot-number "TOY-2026-002" :safety-test-pass-percent 99
                :quantity-units 80.0 :weight-grams 48000.0
                :defect-rate-percent 0.2
                :verified? true :registered? true
                :shipped-units 75.0
                :last-assessed "2026-06-01"}
   "batch-003" {:id "batch-003" :product-type :wooden-toy
                :lot-number "TOY-2026-003" :safety-test-pass-percent 90
                :quantity-units 400.0 :weight-grams 120000.0
                :defect-rate-percent 1.5
...
(defn- schedule-maintenance!
  ...
  maintenance-schedule record via `toysmfg.registry` and returns
...
(defn- propose-shipment!
  ...
  shipment-coordination record via `toysmfg.registry` and returns
  {:result .. :patch ..} for the caller to persist."
  [s shipment-id]
