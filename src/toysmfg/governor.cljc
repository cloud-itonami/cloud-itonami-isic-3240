(ns toysmfg.governor
  "Toys & Games Plant Operations Governor -- the independent
  compliance layer that earns the ToysGamesAdvisor the right to
  ...
  secretly tries to self-issue a toy-safety compliance certification
  ...
  proposal and becomes direct molding/assembly-line-equipment control,
  so this MUST be a separate system able to *reject* a proposal and
  fall back to HOLD.

  `:itonami.blueprint/governor` is
  `:toys-games-plant-operations-governor` (see
  ...
  gate (SOFT -- asks a human to look, and the human may approve):
   ...
   10. Invalid product-type        -- for `:log-production-batch`, if
                                       ...
                                       closed known set
                                       (`toysmfg.registry/product-
                                       type-valid?`), the batch record
                                       is rejected rather than let an
                                       invented classification through.
   11. Invalid safety-test-pass-
       percent                      -- for `:log-production-batch`, if
                                       the patch declares a
                                       `:safety-test-pass-percent` that
                                       is not a physically/certifiably
                                       plausible toy-safety compliance
                                       reading
                                       (`toysmfg.registry/safety-test-
                                       pass-percent-valid?`), the batch
   ...
   14. Confidence floor / high-
       stakes gate                  -- LLM confidence below threshold,
                                       ...
                                       human may approve."
  (:require [toysmfg.registry :as registry]
...
(defn- safety-certification-authority-blocked-violations
  ...
  attempting to self-issue a toy-safety compliance certification (e.g.
  ASTM F963 / EN 71) -- an authority exclusively reserved to the
  accredited testing/certification body, never this actor. No phase
  and no human approval can ever override this."
  [proposal]
  (let [payload (or (:value proposal) (:patch proposal))]
    (when (true? (:issue-safety-certification? payload))
      [{:rule :safety-certification-authority-blocked
        :detail "self-issuing a toy-safety compliance certification (e.g. ASTM F963 / EN 71) is permanently blocked -- exclusive authority of the accredited testing/certification body"}])))
...
(defn- invalid-safety-test-pass-percent-violations
  "For `:log-production-batch`, if the patch declares a
  `:safety-test-pass-percent` that is not a physically/certifiably
  plausible toy-safety compliance reading, reject rather than let
  fabricated/sensor-error data through."
  [{:keys [op]} proposal]
  (when (= op :log-production-batch)
    (let [rating (:safety-test-pass-percent (:value proposal))]
      (when (and (some? rating) (not (registry/safety-test-pass-percent-valid? rating)))
        [{:rule :invalid-safety-test-pass-percent
          :detail (str rating " is outside the physically/certifiably plausible safety-test-pass-percent range")}]))))
...
(defn check
  "Censors a ToysGamesAdvisor proposal against the governor rules.
  ...
  (let [hard (into []
                   (concat (no-propose-effect-violations request)
                           ...
                           (invalid-safety-test-pass-percent-violations request proposal)
